...
Due to limited access to other devices, the application was only tested on a laptop device with the software and hardware configuration specified in table \ref{testing_specs}.
...
Listings \ref{camera_pre} and \ref{camera_post} show the camera movement logic before and after the fix.

% TODO: pseudokod?

\begin{lstlisting}[basicstyle=\ttfamily, showstringspaces=false, caption=Original camera movement implementation., label=camera_pre]
...
\begin{lstlisting}[basicstyle=\ttfamily, showstringspaces=false, caption=Fixed camera movement implementation., label=camera_post]
if move_up and camera_angle - rotate_amount > -PI / 2.0:
...
Figures \ref{basis_pre} and \ref{basis_post} show the \textit{Stacked Cubes} task before and after the fix.
