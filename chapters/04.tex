...
The application was only tested on a laptop device with the software and hardware configuration specified in table \ref{testing_specs}.

\begin{table}%[ht]
    \centering
    \caption{Software and hardware configuration of the device on which the application was tested.}
    \label{testing_specs}
...
Table \ref{godot_specs} presents the minimum system requirements for a simple 2D or 3D project as specified by the Godot Engine documentation \cite{godotspecsdocs}.

\begin{table}[H]
    \centering
    \caption{Minimum system requirements for a simple 2D or 3D project specified by the Godot Engine documentation.}
    \label{godot_specs}
    \begin{tabularx}{0.9\linewidth}{|l|X|}
        \hline
...
The main menu contains several buttons used to navigate the application:
\begin{itemize}
...
When an object (the solid, one of its vertices or one of its edges) is hovered over by the mouse cursor, it becomes red.
...
\begin{itemize}
    ...
    \item A list of all of the steps required to solve the tasks.
        ...
        \begin{itemize}
            \item The description. It specifies what should be calculated for the step.
            \item A help button. The button shows a hint for its respective step when pressed.
            \item A spin box\footnote{A spin box or spinner is a user interface element which allows the user to adjust a value using a text box or up and down arrows.}. It displays the correct answer for its respective step.
        \end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Task exploration module. The \textit{Cube} task is selected. The \textit{Side} parameter has a value of 3.}
    \label{explore_cube}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Task exploration module. The \textit{Cube} task is selected. The \textit{Side} parameter has a value of 5.}
    \label{explore_cube2}
\end{figure}
...
Inside the main part of the task solving module, the user can find the following UI elements:
\begin{itemize}
    \item 3D view showing the current task
    \item description of the task
    \item step navigation buttons
    \item \textit{step list}
    \item \textbf{Check answer} button
    \item \textbf{Get new task} button
\end{itemize}
...
If the user enters the correct value and presses the \textbf{Check answer} button, a green check mark appears to the right side of the spin box (see figure \ref{task_cube_first_step_good}).
...
\begin{figure}[H]
    ...
    \caption{A green check mark appears next to the spin box when the correct answer is provided.}
...
The location of the \textbf{settings.cfg} file is platform-dependent (see table \ref{settings_file_loc}).

\bigskip

\begin{table}[H]
    \centering
    \caption{Location of the \textbf{settings.cfg} file depending on the platform.}
    \label{settings_file_loc}
    \begin{tabularx}{0.9\linewidth}{|l|X|}
        \hline
        Platform & Location \\
        \hline
